...
本项目受 [alchaincyf/nuwa-skill](https://github.com/alchaincyf/nuwa-skill) 启发,并围绕**反幻觉**做了一次重构。

| | 🪶 nuwa-skill | 🧬 distill-forge |
|---|---|---|
| 🛡️ **反幻觉** | agent 自报"一手",可被编造 | **provenance 指纹**:一手必须绑定存档原文 + 逐字 `span`,脚本自动核验、对不上就降级 |
| 🌍 **任何人** | 示例全是预训练海量覆盖的名人 | **`fame_tier` 数据密度自适应**:名人加冷召回防污染,小众高手诚实标低置信度 |
| 🎭 **有用性** | 硬编码"变成那个人" | **`use_case` 双模板**:沉浸扮演 / 顾问(显式亮出 模型+启发+局限) |
| ⚖️ **伦理** | 一句免责声明 | **`target_class` + consent 闸门**:私人 / 逝者特殊处理,出处随 skill 流通 |
| 🔁 **生命周期** | 构建时验一次 | **可重跑 eval**:换模型 / 更新后查"人格漂移" |
